...
class RmaGridDataProvider extends \Magento\Framework\View\Element\UiComponent\DataProvider\DataProvider
{
    ...
    {
        $groups     = [];
        $fieldValue = $fieldExchangeOrderValue = '';
        ...
        if ($fieldExchangeOrderValue) {
            $collection->getSelect()->where(
                'EXISTS(
                    SELECT *
                    FROM mage_mst_rma_rma_order AS `rma_order_table`
                	INNER JOIN mage_sales_order sorder ON sorder.entity_id = rma_order_table.re_exchange_order_id
                	    AND sorder.increment_id = ?
