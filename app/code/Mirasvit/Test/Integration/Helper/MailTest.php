...
class MailTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->helper = $this->objectManager->create('Mirasvit\Rma\Helper\Mail');
    }
     ...
     *
     * @covers  Mirasvit\Rma\Helper\Mail::sendNotificationCustomerEmail
     */
    public function testSendNotificationCustomerEmail()
    {
        /** @var \Mirasvit\Rma\Model\Rma $rma */
        $rma = $this->objectManager->create('Mirasvit\Rma\Model\Rma')->load(1);

        $this->helper->sendNotificationCustomerEmail($rma, 'my message');

        /** @var \Magento\TestFramework\Mail\Template\TransportBuilderMock $transportBuilder */
        $transportBuilder = $this->objectManager->get('Magento\TestFramework\Mail\Template\TransportBuilderMock');
        $actual = $this->htmlToText($transportBuilder->getSentMessage()->getBodyHtml()->getRawContent());

        $this->assertEquals(
            $this->getExpectedMail('notification_customer_email'),
            $actual
        );
    }
     ...
     * @magentoDataFixture Mirasvit/Rma/_files/rma.php
     *
     * @covers  Mirasvit\Rma\Helper\Mail::sendNotificationAdminEmail
     */
    public function testSendNotificationAdminEmail()
    {
        /** @var \Mirasvit\Rma\Model\Rma $rma */
        $rma = $this->objectManager->create('Mirasvit\Rma\Model\Rma')->load(1);

        $this->helper->sendNotificationAdminEmail($rma, 'my message');

        /** @var \Magento\TestFramework\Mail\Template\TransportBuilderMock $transportBuilder */
        $transportBuilder = $this->objectManager->get('Magento\TestFramework\Mail\Template\TransportBuilderMock');
        $actual = $this->htmlToText($transportBuilder->getSentMessage()->getBodyHtml()->getRawContent());
        //magento bug. different formats on linux and mac
        $actual = str_replace("Jul 2, 2015 5:00 PM", "Jul 2, 2015, 5:00 PM", $actual);
        $actual = str_replace("Jul 4, 2015 5:00 PM", "Jul 4, 2015, 5:00 PM", $actual);

        $this->assertEquals(
            $this->getExpectedMail('notification_admin_email'),
            $actual
        );
    }
