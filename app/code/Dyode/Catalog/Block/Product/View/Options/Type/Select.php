 ...
 *
 * Replicating the class Magento\Catalog\Block\Product\View\Options\Type in order to provide unique ids for
 * warranty radio inputs, which will otherwise conflict with the options appear in the product details page.
 */
    ...
    public function getValuesHtml()
    {
        ...
        if ($_option->getType() == \Magento\Catalog\Api\Data\ProductCustomOptionInterface::OPTION_TYPE_RADIO ||
                ...
                case \Magento\Catalog\Api\Data\ProductCustomOptionInterface::OPTION_TYPE_RADIO:
                    ...
                    if (!$_option->getIsRequire()) {
                        $selectHtml .= '<div class="field choice admin__field admin__field-option">' .
                            '<input type="radio" id="warranty_options_' .
                            $_option->getId() .
                            '" class="' .
                            $class .
                            ...
                            ' value="" checked="checked" /><label class="label admin__field-label" for="warranty_options_' .
                            $_option->getId() .
                            '"><span>' .
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                $selectHtml .= '<div class="field choice admin__field admin__field-option'
                    . $require . '">' . '<input type="' . $type . '" class="' . $class . ' '
                    . $require . ' product-custom-option"'
                    . ($this->getSkipJsReloadPrice() ? '' : ' onclick="opConfig.reloadPrice()"')
                    . ' name="options[' . $_option->getId() . ']' . $arraySign . '" id="warranty_options_'
                    . $_option->getId() . '_' . $count . '" value="' . $htmlValue . '" ' . $checked
                    . ' data-selector="' . $dataSelector . '"' . ' price="'
                    . $this->pricingHelper->currencyByStore($_value->getPrice(true), $store, false)
                    . '" />' . '<label class="label admin__field-label" for="warranty_options_' . $_option->getId()
                    . '_' . $count . '"><span>' . $_value->getTitle() . '</span> ' . $priceStr . '</label>';
